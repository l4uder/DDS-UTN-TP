...
public class AsignadorDonacion {
  private List<AlgoritmoMatchmaking> algoritmosMatch;

  public AsignadorDonacion() {
  ...
  public List<ResultadoAsignacion> asignar(List<Donacion> donaciones, List<Beneficiario> beneficiarios) {
    return donaciones.stream().map(d -> new ResultadoAsignacion(d, asignar(d, beneficiarios))).toList();
  }
