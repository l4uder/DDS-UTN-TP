...
public class Donacion {
    ...
    public void notificarEntregaFallida(String observacion) {
        if (observacion == null || observacion.isBlank()) {
            throw new DomainValidationException("Se requiere justificación para notificar entrega fallida");
        }
        if (getEstadoActual() == TipoEstadoDonacion.ENTREGA_FALLIDA) {
            return;
        }
        if (getEstadoActual() != TipoEstadoDonacion.EN_TRASLADO) {
            throw new CambioDeEstadoNoPermitidoException("La donacion debe estar en traslado para notificar entrega fallida");
        }
        this.historialEstados.add(new EstadoDonacion(TipoEstadoDonacion.ENTREGA_FALLIDA, observacion));
    }
    ...
    public void confirmarEntrega() {
        ...
        if (getEstadoActual() != TipoEstadoDonacion.EN_TRASLADO) {
            throw new CambioDeEstadoNoPermitidoException("La donacion debe estar en traslado para confirmar entrega");
        }
    ...
    public void confirmarTrasladoEnCurso() {
        ...
        if (getEstadoActual() != TipoEstadoDonacion.LISTA_PARA_ENTREGAR) {
            throw new CambioDeEstadoNoPermitidoException("La donacion debe estar lista para entregar para iniciar el traslado");
        }
    ...
    public void confirmarAsignacion(EntidadBeneficiaria beneficiario) {
        ...
        this.historialEstados.add(new EstadoDonacion(TipoEstadoDonacion.ASIGNACION_REALIZADA, "Se realizó la asignación a " + beneficiario.getRazonSocial()));
    }
    ...
    public void confirmarRecepcionDeposito() {
        ...
        if (getEstadoActual() != TipoEstadoDonacion.ENTREGA_FALLIDA) {
            throw new CambioDeEstadoNoPermitidoException("No se puede recivir en deposito a menos que la entrega fracase");
        }
        this.historialEstados.add(new EstadoDonacion(TipoEstadoDonacion.EN_DEPOSITO));
    }
