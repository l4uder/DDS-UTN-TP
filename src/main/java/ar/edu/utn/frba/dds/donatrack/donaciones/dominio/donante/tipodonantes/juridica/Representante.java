...
public class Representante {
  ...
  private void chekDatos(String nombre, List<MedioContacto> contactos) {
    if (nombre == null || nombre.isBlank()) throw new DomainValidationException("El campo nombre es obligatorio, en representante");
    if (contactos == null || contactos.isEmpty()) {
      throw new DomainValidationException("Debe proporcionar al menos un contacto, en representante");
    }
    if (contactos.stream().noneMatch(MedioContacto::getEsPrincipal)) {
      throw new DomainValidationException("Debe tener al menos un contacto principal, el representante");
    }
  }
