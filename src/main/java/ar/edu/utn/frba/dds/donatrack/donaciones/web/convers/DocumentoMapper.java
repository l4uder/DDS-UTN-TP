...
public class DocumentoMapper {
  ...
  private static TipoDocumento aTipoDocumento(String tipoDocumento, TipoPersona tipoPersona) {
    if (tipoDocumento == null || tipoDocumento.isBlank()) {
      throw new DomainValidationException("El 'tipo' de un documento es obligatorio, debe ser alguno de: " + TipoDocumento.values(tipoPersona));
    }
    try {
      return TipoDocumento.valueOf(tipoDocumento.toUpperCase());
    } catch (IllegalArgumentException e) {
      throw new DomainValidationException("El tipo de documento: " + tipoDocumento + " no existe, debe ser alguno de: " + TipoDocumento.values(tipoPersona));
    }
  }
