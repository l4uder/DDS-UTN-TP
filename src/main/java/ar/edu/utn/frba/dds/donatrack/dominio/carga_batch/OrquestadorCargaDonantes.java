...
public class OrquestadorCargaDonantes {
    ...
    public static ResultadoImportacion iniciarCarga(String pathArchivoCsv) {
        ...
        InputStream csvStream = OrquestadorCargaDonantes.class.getClassLoader().getResourceAsStream(pathArchivoCsv);
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(csvStream, StandardCharsets.UTF_8))) {
            ...
            while (iter.hasNext()) {
                ...
                try {
                    donanteNuevo = DonanteFactory.crear(resultado.datosDonante().tipoPersona(), resultado.datosDonante().documento(),
                            resultado.datosDonante().nombreCompleto(), resultado.datosDonante().contactoPrincipal(),
